...
def End_windows(screen, size, message, running):
    screen = pg.display.set_mode(size, pg.RESIZABLE)
    caption = pg.display.set_caption("Kéo Búa Bao")
    run = True
    while run:
        screen.fill(white)
        screen.blit(font_End.render(message, True, red), (180, 50))
...
while running:
    ...
    Player_guess_image_not_rotate = pg.transform.scale(Plr_guess_image, (450, 400))
    Player_guess_image = pg.transform.rotate(Player_guess_image_not_rotate, 90)
    ...
    if save_image:
        ...
        pg.image.save(Player_guess_image_not_rotate, Saved_Image +"\\" + name ) 
    ...
    screen.blit(font_CountDown.render(text, True, red), coor_CountDow)
    ...
    if ( Player_Score < Computer_Score and Computer_Score == 5):
        running = End_windows(screen, (700, 400), Message[2], running)
        Player_Score = 0
        Computer_Score = 0
        
    if (Computer_Score < Player_Score and Player_Score == 5):
        running = End_windows(screen, (700, 400), Message[1], running)
        Player_Score = 0
        Computer_Score = 0
